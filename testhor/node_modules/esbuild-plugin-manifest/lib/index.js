...
const generateEntriesResultObject = (fullPathToManifest, mappings, options) => {
    // If the append option is used, we'll read the existing manifest file and merge it with the new entries.
    let existingManifest = {};
    if (options.append) {
        try {
            existingManifest = JSON.parse(fs_1.default.readFileSync(fullPathToManifest, 'utf8'));
        }
        catch (e) { }
    }
...
const writeFileWithLock = async (fullPath, mappings, options) => {
    ...
    setup(build) {
        ...
        build.onEnd((result) => {
            ...
            for (const outputFilename in result.metafile.outputs) {
                ...
                if (path_1.default.parse(path_1.default.parse(key).name).name === 'chunk') {
                    key = outputFilename;
                }
            ...
            if (build.initialOptions.write === false) {
                ...
                (_a = result.outputFiles) === null || _a === void 0 ? void 0 : _a.push({
                    ...
                    hash: crypto_1.default.createHash('sha256').update(text).digest('hex'),
                    get text() {
                        return text;
                    }
                });
